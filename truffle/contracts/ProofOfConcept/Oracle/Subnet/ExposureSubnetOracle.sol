...
contract ExposureSubnetOracle is Ownable {
    ...
    address public exposureSubnetBridgeManager;
    ...
    constructor(address bridgeManager) {
        exposureSubnetBridgeManager = bridgeManager;
    }

    function changeBridgeAddress(address newAddress) external onlyOwner {
        exposureSubnetBridgeManager = newAddress;
    }

    function updateMultiple(address[] memory tokens, uint256[] memory prices) external {
        require(tokens.length == prices.length, "Invalid lengths");
    ...
    function updatePrice(address mainnetToken, uint256 _price) public onlyOwner returns (uint256) {
        address subnetTokenAddress = IExposureSubnetBridge(exposureSubnetBridgeManager).subnetAddresses(mainnetToken);
    ...
    function updateMultipleMarketCap(address[] memory tokens, uint256[] memory mcaps) external {
        require(tokens.length == mcaps.length, "Invalid lengths");
